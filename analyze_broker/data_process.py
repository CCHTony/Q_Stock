...
# 不需要时间范围限制，分析从第一笔数据开始
dates = sorted(data['date'].dt.date.unique())
print(f"分析的日期范围从 {dates[0]} 到 {dates[-1]}，共 {len(dates)} 天。")

# 所有券商的列表
brokers = data['securities_trader'].unique()

# # 初始化一个列表，用于存储 '統一板橋' 的持倉記錄
# unified_baQiao_records = []
        ...
        for entry in history:
            ...
            for price, qty in holdings.items():
                ...
                if qty != 0:
                    ...
                    #                     # 如果是 '統一板橋'，将记录添加到 unified_baQiao_records
                    # if broker == '統一板橋':
                    #     unified_baQiao_records.append(record)
...
# # 将 '統一板橋' 的持倉記錄保存為 CSV
# if unified_baQiao_records:
#     unified_baQiao_df = pd.DataFrame(unified_baQiao_records)
#     # 确保输出目录存在
#     import os
#     output_csv_path = f'analyze_broker/analyzeData/{stock_code}_統一板橋.csv'
#     os.makedirs(os.path.dirname(output_csv_path), exist_ok=True)
#     unified_baQiao_df.to_csv(output_csv_path, index=False, encoding='utf-8-sig')
#     print(f"'統一板橋' 的持倉數據已保存到 {output_csv_path}")
# else:
#     print("沒有找到 '統一板橋' 的持倉記錄。")
