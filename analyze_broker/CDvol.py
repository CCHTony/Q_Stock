...
# 設定累積天數
accumulate_days = 22  # 您可以根據需要自行設定
...
# 遍歷每個選定的日期，並為每個日期生成累積N天的圖表
for current_date in filtered_dates:
    try:
        # 找到當前日期在所有日期中的索引
        current_index = all_dates.index(current_date)
    except ValueError:
        print(f"日期 {current_date} 不存在於數據集中。跳過此日期。")
        continue

    # 獲取當前日期及前(N-1)個交易日的索引
    start_index = max(0, current_index - (accumulate_days - 1))
    cumulative_indices = range(start_index, current_index + 1)
    ...
    fig.update_layout(
        barmode='relative',
        title=dict(
            text = f'{current_date} 及前{accumulate_days - 1}個交易日 ({cumulative_dates[0]} ~ {cumulative_dates[-1]}) 券商累積買入和賣出量分佈', 
            x=0.5,
            xanchor='center',
    ...
    # 等待用戶輸入以繼續
    input(f"已顯示 {current_date} 及前{accumulate_days - 1}個交易日的圖表。按 Enter 繼續到下一個圖表...")
