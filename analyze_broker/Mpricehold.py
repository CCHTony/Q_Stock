...
end_date = datetime.strptime('2024-09-21', '%Y-%m-%d').date()
...
# 步驟 4.1：新增 'month' 列，表示每筆交易的月份（格式為 YYYY-MM）
data['month'] = data['date'].dt.to_period('M').dt.to_timestamp()

# 步驟 5：按月、價格和券商分組，計算每個券商在每個價格和月份下的買入、賣出和淨持有量
grouped = data.groupby(['month', 'price', 'securities_trader']).agg({
    'buy': 'sum',
    'sell': 'sum',
    'net_hold': 'sum'
}).reset_index()
...
# 步驟 6：獲取所有唯一的月份
months = grouped['month'].dt.date.unique()

# 步驟 7：定義要顯示的前N名
TOP_N = 5

# 步驟 9：篩選月份範圍內的數據
filtered_months = [month for month in months if start_date <= month <= end_date]
...
for month in filtered_months:
    # 步驟 10：選擇當月的數據
    monthly_data = grouped[grouped['month'].dt.date == month]
    
    # 獲取當月的所有價格，並排序
    prices = sorted(monthly_data['price'].unique())
    
    # 計算每個價格的總 net_hold
    price_total_net_hold = monthly_data.groupby('price')['net_hold'].sum().to_dict()
    ...
    for price in prices:
        price_data = monthly_data[monthly_data['price'] == price]
    ...
    fig.update_layout(
        barmode='relative',
        title=dict(
            text=f'{month.strftime("%Y-%m")} 券商淨持有量分佈',
            x=0.5,
            xanchor='center',
    ...
    # 等待用戶輸入以繼續
    input(f"已顯示 {month.strftime('%Y-%m')} 的圖表。按 Enter 繼續到下一個圖表...")
